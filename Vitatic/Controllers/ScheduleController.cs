...
public class ScheduleController : ControllerBase
{
   ...
   // }
    ...
    public async Task<ActionResult<ICollection<Activity>>> GetActivities(int id)
    {
        ...
        response = await _context.Activities.Where(a => a.ScheduleId == id).OrderBy(p => p.Priority).OrderBy(d => d.Date).ToListAsync();
        return Ok(response);
    }
    [HttpGet]
    public async Task<ActionResult<ICollection<Activity>>> GetAllActivities()
    {
        ICollection<Activity> response;
    ...
    public async Task<ActionResult<Activity>> DeleteActivity(int id)
    {
        ...
        var progreso = await _context.Progresses.FindAsync(id);
        if (entity == null)
        {
            return NotFound($"Activity with Id = {id} not found");
        }
        _context.Entry(entity).State = EntityState.Deleted;
        _context.Entry(progreso).State = EntityState.Deleted;
        await _context.SaveChangesAsync();
        return Ok();
    }
}
